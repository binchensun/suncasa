...
def add_preset_roi_selection(self, preset=None):
    if preset == 'Img_Inte_ROIs':
        ...
        add_new_group(self)
        freq_boundry = np.linspace(0, len(self.cfreqs), num_of_rois + 1, dtype=int)
        freq_indexs = freq_boundry[0:-1]
        size_list = [int(max(150.0 * self.cfreqs[0] / self.cfreqs[freq_ind], 5.)) for freq_ind in freq_indexs]
